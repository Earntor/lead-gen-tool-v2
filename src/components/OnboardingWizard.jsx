...
export default function OnboardingWizard({ open, onClose, onComplete }) {
  ...
  // 2) zo nodig geforceerd refreshen + retry
    ...
    <div className="absolute inset-0 z-10 flex items-center justify-center p-3 sm:p-6 pointer-events-none">
      ...
      <div className="relative z-20 w-full max-w-xl rounded-2xl bg-white shadow-xl border pointer-events-auto">
        ...
        <div className="p-4 sm:p-6">
          {/* --- JOUW BESTAANDE STEP-CONTENT ONGEWIJZIGD LATEN --- */}
          {step === 1 && (
            <>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                <div>
                  ...
                  <input type="text" required value={firstName} onChange={(e) => setFirstName(e.target.value)} placeholder="Voornaam" className="w-full rounded-lg border px-3 py-2 text-sm" />
                </div>
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-1">Achternaam <span className="text-red-500">*</span></label>
                  <input type="text" required value={lastName} onChange={(e) => setLastName(e.target.value)} placeholder="Achternaam" className="w-full rounded-lg border px-3 py-2 text-sm" />
                </div>
              </div>
              <div className="mt-3">
                <label className="block text-sm font-medium text-gray-700 mb-1">Telefoon (optioneel)</label>
                <input type="tel" value={phone} onChange={(e) => setPhone(e.target.value)} placeholder="+31 6 12 34 56 78" className="w-full rounded-lg border px-3 py-2 text-sm" />
              </div>
          ...
          {step === 2 && (
            <>
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-2">
                {roleOptions.map((opt) => (
                  <button type="button" key={opt} onClick={() => setRole(opt)}
